...
| Translating the bar widget | `locale/<code>.panel.strings`, beside the other table and in the same format. The panel is QML and cannot call `t()`, so it runs `bin/imthemousenow-strings panel` when it opens and looks strings up with `root.str(key, english)` -- same fallback contract, English at every call site. A new string means a key in the file AND the call in `shell/Panel.qml`: `tests/locale-panel.sh` fails if either is missing, or if the two Englishes stop matching. Option values (`hints`, `grid`, ...) are not translatable, for the same reason ACTION words are not. |
