...
| A failure nobody can see | `notify_launch_failure` in `bin/imthemousenow-lib.sh` decides what is said; `bin/imthemousenow-diagnose` builds the prompt the notification's click hands to Omarchy's agent. What makes it fire is the `err:` grep in `run_once` in `bin/imthemousenow`: wl-kbptr's stderr goes to `$LAUNCH_LOG`, and `err:` is the one level that means it refused to start. `info:` and `warn:` are a normal run, and Escape exits nonzero, so neither of those may trigger it. `tests/launch-failure.sh`. |
...
| Translating a string (key sheet, modifier prefix) | `locale/<code>.strings`, one flat `key = "value"` file per locale, copied from `locale/en.strings` and only the right-hand side changed. Looked up by `t()` in `bin/imthemousenow-lib.sh`; every call site also carries the English as a fallback, so a partial locale never shows a blank. The ACTION word itself (`config.default.toml`'s `label`) isn't translatable yet -- it doubles as that action's config key, so a translator's file has no effect on it until that's split. `--help` heredocs and `docs/manual/` are untouched by this and stay English-only for now. `locale/` is in `install.sh`'s `MANAGED` list, so a dev install symlinks it and edits are live; a copying install needs a re-run. |
| Translating the bar widget | `locale/<code>.panel.strings`, beside the other table and in the same format. The panel is QML and cannot call `t()`, so it runs `bin/imthemousenow-strings panel` when it opens and looks strings up with `root.str(key)` -- and, unlike `t()`, with NO English at the call site. The table is the only place the panel's words exist: a key it does not have renders as the bracketed key name and warns once, and a table that failed to load puts a message across the top of the panel. That is the point -- a fallback copy made a broken table read as a perfect English panel no locale could reach. A new string means a key in the file AND the call in `shell/Panel.qml`; rewording an existing one needs only the file. `tests/locale-panel.sh` fails if a key is missing on either side, or if a call site grows an English fallback back. Option values (`hints`, `grid`, ...) are not translatable, for the same reason ACTION words are not. |
