// The mark the pointer wears for as long as a hold holds the button down.
//
...
// and a mouse button is down. This mark is the only thing that says so, so it
// pulses rather than sitting still: a static mark reads as decoration, and
// movement is what makes it read as a state.
//
// It is the same mark a click leaves (qml/PoolSpot.qml), which is the point: a
// hold and a click say "pressed here" in one language rather than two. It used
// to be a set of pulsing rings, with the mark drawn over them once the mark
// existed; the rings under it were then decoration hidden by the thing that
// replaced them, so they are gone. Unlike the click's mark, this one is not
// governed by `pool.enabled` -- that setting turns off a flourish left behind
// by a click, while this is the only report that a button is down.
//
...
//      were not there. Without it the mark would swallow the drag it is
//      drawing.
...
ShellRoot {
  ...
  // What the file last said, kept apart from where the mark is drawn so the
  // two can be compared.
  ...
  // the mouse -- the button stays down, the drag goes on -- and a mark read
  ...
  //
  // With no Hyprland socket to ask, `tracking` never comes on and the mark
  ...
  FileView {
    ...
    // the mark follow the pointer rather than sit where the hold started.
    watchChanges: true
  ...
  // when it happens -- and a mark left behind at the last position would be
  // pointing at somewhere the pointer no longer is, which is worse than no mark
  ...
  // and the mark has to be able to follow it there. It is only visible on the
  // screen the pointer is actually on; the others are drawing nothing.
  Variants {
    ...
    PanelWindow {
      ...
      PoolSpot {
        id: spot
        // Off this screen, or nothing read yet: draw nothing rather than a mark
        ...
        // The mark keeps up with the pointer rather than teleporting with it:
        // wl-kbptr walks each move in a few steps, and a mark that jumped the
        // whole distance would arrive before the thing being dragged does.
        Behavior on centerX { NumberAnimation { duration: 40; easing.type: Easing.OutQuad } }
        Behavior on centerY { NumberAnimation { duration: 40; easing.type: Easing.OutQuad } }
      }
    }
  }
}
