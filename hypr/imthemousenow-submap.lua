-- imthemousenow: the submap the overlay itself runs in.
--
-- Everything an open overlay needs -- ACTION switches, swap, resize, the key
-- sheet, the panic key, popup-safe relaying -- is wired up by this file alone.
-- Nothing here opens the overlay: there is no SUPER + ; in this file. That is
-- the point of splitting it out of hypr/imthemousenow.lua, which requires this
-- module and adds the entry chords on top.
--
-- install.sh offers three ways to wire this plugin into Hyprland, controlled
-- by --keybinds full|submap|none:
--
--   full    require("omarchy.plugins.imthemousenow.hypr.imthemousenow"),
--           which requires this file too -- SUPER + ; and friends, ready to
--           use.
--   submap  require("omarchy.plugins.imthemousenow.hypr.imthemousenow-submap")
--           directly, with no entry chord at all -- for wiring your own key to
--           `imthemousenow` (or `imthemousenow --flip ...`) in
--           ~/.config/hypr/bindings.lua, while still getting a working
--           overlay, a working panic key and a working Escape out of it.
--   none    neither file loaded. Nothing here runs, including the panic key,
--           so read the warning in install.sh and in
--           docs/manual/02-keybindings.md before choosing it -- with
--           [imthemousenow.popups] keep_open on (the shipped default) an
--           overlay opened with no submap loaded cannot be reached by the
--           keyboard AT ALL, because that mode asks for no keyboard focus of
--           its own and relays every key through binds this file provides.
--
-- Override any one bind in ~/.config/hypr/bindings.lua by calling
-- hl.unbind("SUPER + SEMICOLON") (or whichever bind) first, then binding it
-- yourself -- see docs/manual/02-keybindings.md, "Bring your own keybinding".
...
--
-- SHIFT and ALT keep the same meanings here as they do in the entry chords
-- (hypr/imthemousenow.lua): tapped on their own, they flip the same axis
-- again, so the overlay you are looking at can become the one you meant
-- without closing it.
--
-- ACTION is not on a modifier because it is decided after you can see the
-- overlay, not before: `;`, `:`, `'` and `"` switch it.
--
...
-- Hyprland keybindings still fire while wl-kbptr holds the keyboard. This
-- lives here, not in the entry file, so that "submap" and "full" installs
-- both get it -- the panic key has to exist whenever an overlay can come up,
-- whichever file opens one.
